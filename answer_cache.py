...
def normalize_question(question: str) -> str:
    ...
    - Remove filler words at start (okay, so, well, um, etc.)

    Args:
        question: raw question text

    Returns:
        normalized question string (cache key)
    """
    if not question:
        return ""
...
def get_cached_answer(question: str) -> Optional[str]:
    ...
    Returns:
        cached answer string, or None if not cached
    """
...
def is_duplicate_question(question: str) -> bool:
    ...
    Use this to detect repeated questions without retrieving the answer.

    Args:
        question: the interview question

    Returns:
        True if question is cached (duplicate)
    """
    key = normalize_question(question)
    if not key:
        return False
